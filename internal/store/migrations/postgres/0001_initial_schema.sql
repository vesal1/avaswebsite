-- Avas Sportsbook: initial schema (PostgreSQL).
--
...
--     Deliberately TEXT rather than TIMESTAMPTZ: the application formats and
--     parses them itself, and keeping one representation across both
--     supported databases removes a whole class of timezone drift between
--     environments. Reporting queries can cast with `col::timestamptz`.
--   * Booleans are stored as INTEGER 0/1, again so both databases agree.
...
CREATE TABLE IF NOT EXISTS player_limits (
    ...
    kind            TEXT    NOT NULL
                            CHECK (kind IN ('deposit_daily', 'deposit_weekly', 'deposit_monthly',
                                            ...
                                            'session_minutes')),
    amount          BIGINT NOT NULL,               -- satoshi, or minutes for session_minutes
...
CREATE TABLE IF NOT EXISTS markets (
    ...
    subject_participant_id BIGINT REFERENCES participants(id) ON DELETE CASCADE,
    status          TEXT    NOT NULL DEFAULT 'open'
                            CHECK (status IN ('open', 'suspended', 'closed', 'settled', 'void')),
    margin_bps      BIGINT NOT NULL DEFAULT 500,
    created_at      TEXT    NOT NULL,
    settled_at      TEXT
);
...
CREATE TABLE IF NOT EXISTS selections (
    ...
    odds_milli      BIGINT NOT NULL CHECK (odds_milli > 1000),
    status          TEXT    NOT NULL DEFAULT 'open'
...
CREATE TABLE IF NOT EXISTS bets (
    ...
    odds_milli      BIGINT NOT NULL CHECK (odds_milli > 1000),
    potential_payout_sat BIGINT NOT NULL CHECK (potential_payout_sat >= 0),
    ...
    accepted_odds_change BIGINT NOT NULL DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS deposits (
    ...
    txid            TEXT    NOT NULL,
    vout            BIGINT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS withdrawals (
    ...
    address         TEXT    NOT NULL,
    amount_sat      BIGINT NOT NULL CHECK (amount_sat > 0),
    fee_sat         BIGINT NOT NULL DEFAULT 0 CHECK (fee_sat >= 0),
